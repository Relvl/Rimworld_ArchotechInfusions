...
public class Printer : AGridBuilding
{
    private AInstruction _instruction;
    private PrinterProps.Comp _printerComp;
    private Thing _targetThing;
    private int _ticksCurrentCycle;

    private PrintWindowSelector _windowSelector;
    public PrinterProps.Comp PrinterComp => _printerComp ??= GetComp<PrinterProps.Comp>();
    ...
    {
        return Grid.Get<InstructionDatabase>().Any(db => db.Instructions.Count > 0);
    }
    ...
    public void DoJobTick(JobDriver driver, Pawn pawn)
    {
        ...
        if (_ticksCurrentCycle >= PrinterComp.Props.PrintTicks)
        {
            ...
            var (damageAmount, breakChance) = comp.GetDamageData(_instruction);
