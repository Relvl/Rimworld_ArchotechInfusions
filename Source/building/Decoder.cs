...
public class Decoder : AGridBuildingStateful
{
    ...
    private DecoderProps.Comp _comp;
    private AInstruction _instruction;
    private bool _mute;
    public DecoderProps.Comp Comp => _comp ??= GetComp<DecoderProps.Comp>();
    ...
    protected override void OnStateComplete()
    {
        switch (State)
        {
            ...
            case StateDecodingKey:
                ...
                {
                    StateError = Grid.Get<InstructionDatabase>().Any() ? "JAI.Error.WaitingForDatabase".Translate() : "JAI.Error.GridHasNoDatabase".Translate();
