...
public class ArchiteRepairer : AGridBuilding
{
    ...
    private ArchiteRepairerProps.Comp _repairerComp;
    ...
    public ArchiteRepairerProps.Comp Comp => _repairerComp ??= GetComp<ArchiteRepairerProps.Comp>();
    ...
    public override IEnumerable<Gizmo> GetGizmos()
    {
        ...
        if (DebugSettings.ShowDevGizmos)
        {
            yield return new Command_Action
            {
                ...
                {
                    foreach (var pawn in Comp.parent.Map.mapPawns.AllPawnsSpawned.Where(c => c.IsColonist))
            ...
            yield return new Command_Action
            {
                defaultLabel = "Log stats", action = () =>
                {
                    ...
                    Log.Warning($"{LabelCap}: Props.HpPerTick: {Comp.Props.HpPerTick}");
                    Log.Warning($"{LabelCap}: Props.ArchitePerHp: {Comp.Props.ArchitePerHp}");
                    Log.Warning($"{LabelCap}: Props.EnergyPerHp: {Comp.Props.EnergyPerHp}");
    ...
    public bool CanWork()
    {
        if (Comp.Props.HpPerTick == 0) return false;
        if (!Grid.PowerOn) return false;
        if (_chargeCache < Comp.Props.EnergyPerHp * 10) return false;
        if (_architeCache < Comp.Props.ArchitePerHp * 10) return false;
        if (!Grid.Get<ArchiteContainer>().Any(c => c.Stored >= Comp.Props.ArchitePerHp)) return false;
        return true;
    }
    ...
    public void DoJobTick(Pawn pawn, JobDriver driver, float speed)
    {
        ...
        var hpPerTick = Comp.Props.HpPerTick * speed;
    ...
    {
        var energy = Comp.Props.EnergyPerHp * hp;
        var archite = Comp.Props.ArchitePerHp * hp;
    ...
    private class RepairerGizmo(ArchiteRepairer repairer) : Gizmo
    {
